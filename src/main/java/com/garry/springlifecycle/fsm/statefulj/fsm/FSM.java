...
import com.garry.springlifecycle.fsm.statefulj.fsm.model.Transition;
import com.garry.springlifecycle.fsm.statefulj.fsm.model.impl.DeterministicTransitionImpl;
import com.garry.springlifecycle.fsm.statefulj.fsm.model.impl.StateImpl;
...
/**
 * The FSM si responsible for the processing the event with the current State and persisting
 * the State with the composite Persistent
 */
public class FSM<T> {
    ...
    private Persistent<T> persistent;
    ...
    public FSM(Persistent<T> persistent) {
        this.persistent = persistent;
    }

    public FSM(String name,Persistent<T> persistent) {
        this.persistent = persistent;
        this.name = name;
    }

    public FSM(String name, Persistent<T> persister, int retryAttempts, int retryInterval) {
        this.name = name;
    ...
    public Persistent<T> getPersistent() {
        return persistent;
    }

    public void setPersistent(Persistent<T> persistent) {
        this.persistent = persistent;
    }
    ...
    public State<T> getCurrentState(T obj) {
        return this.persistent.getCurrent(obj);
    }
    ...
    public static class FSMBuilder<T> {
        ...
        public static class StateBuilder<T> {
            ...
            private List<TransitionBuilder<T>> transitionBuilders = new LinkedList<TransitionBuilder<T>>();
            ...
            public StateBuilder<T> addTransition(String event, String toState) {
                this.transitionBuilders.add(new TransitionBuilder<T>(event, toState));
                return this;
            }
            ...
            public StateBuilder<T> addTransition(String event, String toState, Action<T> action) {
                this.transitionBuilders.add(new TransitionBuilder<T>(event, toState, action));
                return this;
            }
            ...
            private State<T> buildState() {
                this.state = new StateImpl<T>(this.stateName, this.transitions, this.isEndState, this.isBlocking);
                return this.state;
            }

            private void buildTransitions(Map<String, State<T>> states) {
                for(TransitionBuilder<T> transitionBuilder : this.transitionBuilders) {
         ...
         * @param persistent The Persister
         * @return FSMBuilder
         */
        public FSMBuilder<T> setPersistent(Persistent<T> persistent) {
            this.persistent = persistent;
            return this;
        }
        ...
        public FSM<T> build() {
            ...
            if (this.persistent == null) {
                this.persistent = new MemoryPersistentImpl<>();
            }
            this.persistent.setStates(this.states.values());
            this.persistent.setStartState(startState);

            return new FSM<T>(this.name, this.persistent, this.retryAttempts, this.retryInterval);
        }
    }
    ...
    protected State<T> transition(T stateful, State<T> current, String event, Transition<T> transition, Object... args) throws RetryException {
        ...
        executeAction(pair.getAction(),
                stateful,
                event,
                current.getName(),
                pair.getState().getName(),
                args);
        return getCurrentState(stateful);
    }
    ...
    protected void setCurrent(T stateful, State<T> current, State<T> next) throws StaleStateException {
        persistent.setCurrent(stateful, current, next);
    }
