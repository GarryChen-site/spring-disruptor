...
public class ModelUtil {
	...
	public static boolean isModel(Class clazz) {
		...
		if (ModelIF.class.isAssignableFrom(clazz) || clazz.isAnnotationPresent(Model.class))
			isModelIF = true;
		return isModelIF;
	}
	...
	public static boolean isCachable(Object model) {
		...
		if (ModelIF.class.isAssignableFrom(model.getClass())) {
			...
			if (modelc.isCacheable())
				isCachable = true;
		} else if (model.getClass().isAnnotationPresent(Model.class)) {
			Model modela = model.getClass().getAnnotation(Model.class);
